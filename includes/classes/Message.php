...
class Message {
	...
	public function getConvosDropdown($data, $limit) {
		...
		$set_viewed_query = mysqli_query($this->con, "UPDATE messages SET is_viewed='yes' WHERE user_to='$userLoggedIn'");
		...
		foreach ($convos as $username) {
			...
			$is_unread_query = mysqli_query($this->con, "SELECT is_opened FROM messages WHERE user_to='$userLoggedIn' AND user_from='$username' ORDER BY id DESC");
			$row = mysqli_fetch_array($is_unread_query);
			$style = ($row['is_opened'] == 'no') ? "background-color: #DDEDFF;" : "";
